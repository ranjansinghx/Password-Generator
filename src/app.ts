...
type CharsetKey = "lower" | "upper" | "digits" | "symbols";
...
const el = {
  password: document.getElementById("password") as HTMLDivElement,
  scramble: document.getElementById("scramble") as HTMLDivElement,
  length: document.getElementById("length") as HTMLInputElement,
  lengthValue: document.getElementById("lengthValue") as HTMLSpanElement,
  entropyFill: document.getElementById("entropyFill") as HTMLDivElement,
  entropyBits: document.getElementById("entropyBits") as HTMLSpanElement,
  entropyLabel: document.getElementById("entropyLabel") as HTMLSpanElement,
  ...
  copyStatus: document.getElementById("copyStatus") as HTMLSpanElement,
  toggles: Array.from(
    document.querySelectorAll<HTMLInputElement>("[data-charset]")
  ),
  ambiguousToggle: document.getElementById("excludeAmbiguous") as HTMLInputElement,
};
...
function activeCharsets(): CharsetKey[] {
  return el.toggles
    .filter((t) => t.checked)
    .map((t) => t.dataset.charset as CharsetKey);
}

function buildPool(keys: CharsetKey[]): string {
...
function generatePassword(length: number, keys: CharsetKey[]): string {
...
function entropyBits(length: number, keys: CharsetKey[]): number {
  const pool = buildPool(keys);
  if (pool.length === 0) return 0;
  return Math.round(length * Math.log2(pool.length));
}

function strengthLabel(bits: number): { label: string; ratio: number; tone: string } {
  if (bits === 0) return { label: "PICK A CHARSET", ratio: 0, tone: "idle" };
  if (bits < 40) return { label: "WEAK", ratio: bits / 120, tone: "weak" };
  if (bits < 65) return { label: "FAIR", ratio: bits / 120, tone: "fair" };
  if (bits < 90) return { label: "STRONG", ratio: bits / 120, tone: "strong" };
...
function revealPassword(target: string) {
...
async function copyPassword() {
  ...
  window.clearTimeout((copyPassword as unknown as { t?: number }).t);
  (copyPassword as unknown as { t?: number }).t = window.setTimeout(() => {
  ...
  window.clearTimeout((copyPassword as unknown as { c?: number }).c);
  (copyPassword as unknown as { c?: number }).c = window.setTimeout(async () => {
    try {
